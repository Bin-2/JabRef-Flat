...
/**
 * Created by IntelliJ IDEA. User: Christoph Arbeit Date: 09.09.2010 Time:
 * 10:56:50 To change this template use File | Settings | File Templates.
 */
public class DocumentsWrapper {
    ...
    public List<List<String>> getDocuments() {
        ...
        if (xmlDocument.getTitle() != null) {
            row.add(xmlDocument.getTitle().getValue());
        } else {
            row.add("");
        }
        ...
        if (xmlDocument.getAuthors() != null) {
            ...
            for (Bean author : authors) {
                value.append(getNameComplete(((Author) author)));
                if (i < authors.size()) {
                    value.append(", ");
                }
                i++;
        ...
        // Year
        if (xmlDocument.getYear() != null
                && ((Year) xmlDocument.getYear()).getValue() != null
                && !((Year) xmlDocument.getYear()).getValue().equalsIgnoreCase("null")) {
            row.add(((Year) xmlDocument.getYear()).getValue());
        } else {
            row.add("");
        }
