...
public class GroupTreeNode extends DefaultMutableTreeNode implements
     ...
     * This is for instance used when updating the group tree due to an external
     * change.
     *
     ...
     * the returned array represent the child index of each node in the path. If
     * this node is the root node, the returned array has zero elements.
     */
    ...
    /**
     * @param indexedPath A sequence of child indices that describe a path from
     * this node to one of its desendants. Be aware that if <b>indexedPath
     * </b> was obtained by getIndexedPath(), this node should usually be the
     * root node.
     * @return The descendant found by evaluating <b>indexedPath </b>. If the
     * path could not be traversed completely (i.e. one of the child indices did
    ...
    public AbstractGroup[] getMatchingGroups(BibtexEntry entry) {
        List<AbstractGroup> matchingGroups = new ArrayList<>();
        Enumeration<GroupTreeNode> e = preorderEnumeration();
        AbstractGroup group;
        while (e.hasMoreElements()) {
