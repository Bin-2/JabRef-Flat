...
public class EntryEditor extends JPanel implements VetoableChangeListener, EntryContainer {
    ...
    private final Map<Component, String> tabIconKeys = new HashMap<>();
...
//    }
    ...
    private void setupFieldPanels() {
        ...
        if (Globals.prefs.getBoolean("showSource")) {
            tabbed.addTab(
                    Globals.lang("BibTeX source"),
                    GUIGlobals.getIcon("source"),
                    srcPanel,
...
//         tlb = new JToolBar(JToolBar.VERTICAL);
        tlb = new JToolBar(JToolBar.VERTICAL) {
            ...
            public void updateUI() {
                super.updateUI();
                // Refresh all button icons after UI update
        ...
        closeBut.setBackground(UIManager.getColor("SplitPane.background"));
        closeBut.setMargin(new Insets(0, 0, 0, 0));
        leftPan.add(closeBut, BorderLayout.NORTH);

        // Create type-label
        // wrap TypeLabel in a panel with bottom padding
        ...
        typePanel.setBorder(BorderFactory.createEmptyBorder(10, 0, 0, 0)); // top gap
        typePanel.add(new TypeLabel(entry.getType().getName()), BorderLayout.CENTER);
        leftPan.add(typePanel, BorderLayout.CENTER);
...
//    }
     ...
     * tabs.Look at EntryEditorTab for the setup of text components in the other
     * tabs.
     *
     * @param ta
     */
    ...
    public synchronized void switchTo(BibtexEntry be) {
        if (entry == be) {
            /**
             * Even if the editor is already showing the same entry, update the
             * source panel. I'm not sure if this is the correct place to do
             * this, but in some cases the source panel will otherwise not be
             * up-to-date when an entry is changed while the entry editor is
             * existing, set to the same entry, but not visible.
             */
            updateSource();
            return;
        }
    ...
    private class TypeButton extends JButton {
        ...
        public void updateUI() {
            ...
            setIcon(GUIGlobals.getImage("edit"));

            // Also refresh any other theme-dependent properties
            setToolTipText(Globals.lang("Change entry type"));
...
//            setForeground(GUIGlobals.entryEditorLabelColor);
        ...
        public void updateUI() {
            ...
            int currentAlignment = getHorizontalAlignment();

            // Update the UI
            super.updateUI();

            // Restore vertical orientation and other custom settings
            setUI(new VerticalLabelUI(false));
            setHorizontalAlignment(currentAlignment);
            setFont(currentFont);

            // Also restore any other custom properties that might be lost
            revalidate();
            repaint();
        }
...
//        }
    ...
    class CloseAction extends AbstractAction {
        ...
        public void updateUI() {
            putValue(SMALL_ICON, GUIGlobals.getImage("close"));
            putValue(SHORT_DESCRIPTION, Globals.lang("Close window"));
            putValue(NAME, Globals.lang("Close window"));
        }
...
//    }
    class NextEntryAction extends ThemedAction {
        ...
        public NextEntryAction() {
            super(Globals.lang("Next entry"), "down");
        }
...
//        }
    ...
    class GenerateKeyAction extends ThemedAction {
...
//        }
    ...
    class UndoAction extends AbstractAction {
        ...
        public UndoAction() {
            super("Undo", GUIGlobals.getIcon("undo"));
    ...
    class RedoAction extends AbstractAction {
        ...
        public RedoAction() {
            super("Undo", GUIGlobals.getIcon("redo"));
            putValue(SHORT_DESCRIPTION, "Redo");
        }

        public void updateUI() {
            putValue(SMALL_ICON, GUIGlobals.getIcon("redo"));
