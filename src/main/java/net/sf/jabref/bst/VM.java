...
public class VM implements Warn {
            ...
            public void execute(BstEntry context) {
                ...
                while (i < n) {
                    ...
                    if (c[i - 1] == '{') {
                        ...
                        if (braceLevel == 1 && i < n) // if (str_pool[sp_ptr] = backslash) then
                        // begin
                        {
                            if (c[i] == '\\') {
                                ...
                                while (i < n && braceLevel > 0) {
                                    ...
                                    if (c[i] == '}') // decr(sp_brace_level)
                                    {
                                        braceLevel--;
                                    } // else if (str_pool[sp_ptr] = left_brace)
                                    // then
                                    else if (c[i] == '{') // incr(sp_brace_level);
                                    {
                                        braceLevel++;
                                    }
                                    // incr(sp_ptr);
                    ...
                    // begin
                        ...
                        if (braceLevel > 0) // decr(sp_brace_level);
                        {
                            braceLevel--;
                        }
                        // end
    ...
    public String run(Collection<BibtexEntry> bibtex) {
        ...
        { // Create entries
            entries = new ArrayList<BstEntry>(bibtex.size());
    ...
    Map<String, String> strings = new HashMap<>();
    Map<String, Integer> integers = new HashMap<>();
    Map<String, BstFunction> functions = new HashMap<>();
