 ...
 *
 * 1) Each OO bib entry type must have a formatting. A formatting is an array of
 * elements, each of which is either a piece of constant text, an entry field
 * value, or a tab. Each element has a character format associated with it.
 *
 * 2) Many field values (e.g. author) need to be formatted before input to
 * OpenOffice. The style has the responsibility of formatting all field values.
 * Formatting is handled by 0-n JabRef LayoutFormatter classes.
 *
 * 3) If the entries are not numbered, a citation marker must be produced for
 * each entry. This operation is performed for each JabRef BibtexEntry.
 */
public class OOBibStyle implements Comparable<OOBibStyle> {
    ...
    /**
     * If this style was initialized from a file on disk, reload the style if
    ...
    /**
     * After initalizing this style from a file, this method can be used to
     * check whether the file appeared to be a proper style file.
     *
     * @return true if the file could be parsed as a style file, false
     * otherwise.
     */
     ...
     *
     * public PropertyValue[][] getReferenceFormat(short bibType) { Object o =
     * bibLayout.get(new Short(bibType)); if (o != null) return
     * (PropertyValue[][])o; else return defaultBibLayout; }
     */
    /**
     ...
     * @param inParenthesis Signals whether a parenthesized citation or an
     * in-text citation is wanted.
     * @param uniquefier String to add behind the year in case it's needed to
     * separate similar entries.
    ...
    /**
     * Format the marker for the in-text citation according to this bib style.
     * Uniquefier letters are added as provided by the uniquefiers argument. If
     * successive entries within the citation are uniquefied from each other,
     * this method will perform a grouping of these entries.
     *
     * @param entries The array of JabRef BibtexEntry providing the data.
     * @param inParenthesis Signals whether a parenthesized citation or an
     * in-text citation is wanted.
     * @param uniquefiers Strings to add behind the year for each entry in case
     * it's needed to separate similar entries.
     * @param unlimAuthors Boolean for each entry. If true, we should not use
     * "et al" formatting regardless of the number of authors. Can be null to
     * indicate that no entries should have unlimited names.
     * @return The formatted citation.
     */
    ...
    /**
     * Modify entry and uniqiefier arrays to facilitate a grouped presentation
     * of uniqiefied entries.
     *
    ...
    /**
     * This method produces (Author, year) style citation strings in many
     * different forms.
     *
     * @param entries The array of BibtexEntry to get fields from.
     * @param authorField The bibtex field providing author names, e.g. "author"
     * or "editor".
     * @param yearField The bibtex field providing the year, e.g. "year".
     * @param maxA The maximum number of authors to write out in full without
     * using etal. Set to -1 to always write out all authors.
     * @param authorSep The String to add between author names except the last
     * two, e.g. ", ".
     * @param andString The String to add between the two last author names,
     * e.g. " & ".
     * @param etAlString The String to represent authors that are not mentioned,
     * e.g. " et al."
     ...
     * @param citationSeparator The String to separate citations from each
     * other.
     ...
     * @param startBrace The opening parenthesis.
     * @param endBrace The closing parenthesis.
     * @param uniquefiers Optional parameters to separate similar citations. Can
    ...
    /**
     * This method looks up a field for en entry in a database. Any number of
     * backup fields can be used if the primary field is empty.
     *
     * @param entry The entry.
     * @param database The database the entry belongs to.
     * @param field The field, or succession of fields, to look up. If backup
     * fields are needed, separate field names by /. E.g. to use "author" with
     * "editor" as backup, specify "author/editor".
     * @return The resolved field content, or an empty string if the field(s)
     * were empty.
     */
    ...
    /**
     * Look up the nth author and return the proper last name for citation
     * markers.
     *
    ...
    /**
     * Take a finished citation and insert a string at the end (but inside the
     * end bracket) separated by "PageInfoSeparator"
     *
     * @param citation
     * @param pageInfo
     * @return
    ...
    /**
     * Convenience method for checking the property for whether we use number
     * citations or author-year citations.
     *
    ...
    /**
     * Convenience method for checking the property for whether we sort the
     * bibliography according to their order of appearance in the text.
     *
    ...
    /**
     * Convenience method for checking whether citation markers should be
     * italicised. Will only be relevant if isFormatCitations() returns true.
     *
     ...
     * according to the results of the isItalicCitations() and isBoldCitations()
     * methods.
     *
     * @return true to indicate that citations should be in italics.
     */
