 ...
 * type, outputting a format string given as the first argument. The format
 * string can contain a number of escape sequences indicating file link
 * information to be inserted into the string.
 * <p/>
 * This formatter can take an optional second argument specifying the name of a
 * file type. If specified, the iteration will only include those files with a
 * file type matching the given name (case-insensitively). If specified as an
 * empty argument, all file links will be included.
 * <p/>
 * After the second argument, pairs of additional arguments can be added in
 * order to specify regular expression replacements to be done upon the inserted
 * link information before insertion into the output string. A non-paired
 * argument will be ignored. In order to specify replacements without filtering
 * on file types, use an empty second argument.
 * <p/>
 * <p/>
 * <p/>
 ...
 * the output list of files should be enumerated. \p : This inserts the file
 * path of the file link. Relative links below your file directory will be
 * expanded to their absolute path. \r : This inserts the file path without
 * expanding relative links. \f : This inserts the name of the file link's type.
 * \x : This inserts the file's extension, if any. \d : This inserts the file
 * link's description, if any.
 * <p/>
 * For instance, an entry could contain a file link to the file
 * "/home/john/report.pdf" of the "PDF" type with description "John's final
 * report".
 * <p/>
 ...
 * <p/>
 * would give the following output: 1. John's final report
 * (/home/john/report.pdf)
 * <p/>
 * If the entry contained a second file link to the file "/home/john/draft.txt"
 * of the "Text file" type with description 'An early "draft"', the output would
 * be as follows: 1. John's final report (/home/john/report.pdf) 2. An early
 * "draft" (/home/john/draft.txt)
 * <p/>
 * If the formatter was called with a second argument, the list would be
 * filtered. For instance: \format[WrapFileLinks(\i. \d (\p),text file)]{\file}
 * <p/>
 * would show only the text file: 1. An early "draft" (/home/john/draft.txt)
 * <p/>
 * If we wanted this output to be part of an XML styled output, the quotes in
 * the file description could cause problems. Adding two additional arguments to
 * translate the quotes into XML characters solves this:
 * \format[WrapFileLinks(\i. \d (\p),text file,",&quot;)]{\file}
 * <p/>
 * would give the following output: 1. An early &quot;draft&quot;
 * (/home/john/draft.txt)
 *
...
public class WrapFileLinks extends AbstractParamLayoutFormatter {
    ...
    /**
     * Parse a format string and return a list of FormatEntry objects. The
     * format string is basically marked up with "\i" marking that the iteration
     * number should be inserted, and with "\p" marking that the file path of
     * the current iteration should be inserted, plus additional markers.
     *
    ...
    /**
     * This class defines the building blocks of a parsed format strings. Each
     * FormatEntry represents either a literal string or a piece of information
     * pertaining to the file link to be exported or to the iteration through a
     * series of file links. For literal strings this class encapsulates the
     * literal itself, while for other types of information, only a type code is
     * provided, and the subclass needs to fill in the proper information based
     * on the file link to be exported or the iteration status.
     */
