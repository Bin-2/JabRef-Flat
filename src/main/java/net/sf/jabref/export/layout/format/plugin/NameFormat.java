...
/**
 * This layout formatter uses the Bibtex name.format$ method and provides
 ...
 *
 * Cases are just integers or the character * and will tell the formatter to
 * apply the following formats if there are less or equal authors given to it.
 * The cases must be in strict increasing order with the * in the last position.
 *
 * For instance:
 *
 * case1 = 2 case2 = 3 case3 = *
 *
 * Ranges are either <integer>..<integer>, <integer> or the character * using a
 * 1 based index for indexing authors from the given authorlist. Integer indexes
 * can be negative to denote them to start from the end of the list where -1 is
 * the last author.
 *
 * For instance with an authorlist of "Joe Doe and Mary Jane and Bruce Bar and
 * Arthur Kay":
 *
 ...
 * are used within curly braces. A single letter v, f, l, j indicates that the
 * name should be abbreviated. To put a quote in the format string quote it
 * using \" (mh. this doesn't work yet)
 *
