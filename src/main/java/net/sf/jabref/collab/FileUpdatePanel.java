...
public class FileUpdatePanel extends SidePaneComponent implements ActionListener,
    ...
    public FileUpdatePanel(JabRefFrame frame, BasePanel panel, SidePaneManager manager, File file,
            ChangeScanner scanner) {
        super(manager, GUIGlobals.getMenuIcon("saveDirty"), Globals.lang("File changed"));
    ...
    /**
     * We include a getter for the BasePanel this component refers to, because
     * this component needs to be closed if the BasePanel is closed.
     *
    ...
    /**
     * Unregister when this component closes. We need that to avoid showing two
     * such external change warnings at the same time, only the latest one.
     */
     ...
     *
     * @param resolved true if there were no changes, or if the user has
     * resolved them.
     */
