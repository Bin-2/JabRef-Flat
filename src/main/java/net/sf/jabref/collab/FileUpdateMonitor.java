...
/**
 * This thread monitors a set of files, each associated with a
 * FileUpdateListener, for changes in the file's last modification time stamp.
 * The
 */
public class FileUpdateMonitor extends Thread {
    ...
    /**
     * Cause the thread to stop monitoring. It will finish the current round
     * before stopping.
     */
     ...
     *
     * @param ul FileUpdateListener The listener to notify when the file
     * changes.
    ...
    /**
     * Forces a check on the file, and returns the result. Does not force a
     * report to all listeners before the next routine check.
     */
     ...
     * have been modified. Used if a file has been modified, and the change scan
     * fails, in order to ensure successive checks.
     *
    ...
    /**
     * Method for getting the temporary file used for this database. The
     * tempfile is used for comparison with the changed on-disk version.
     *
     * @param key String The handle for this monitor.
     * @throws IllegalArgumentException If the handle doesn't correspond to an
     * entry.
    ...
    /**
     * A class containing the File, the FileUpdateListener and the current time
     * stamp for one file.
     */
    class Entry {
