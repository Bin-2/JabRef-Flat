...
public class ChangeScanner extends Thread {
    ...
    /**
     * We create an ArrayList to hold the changes we find. These will be added
     * in the form of UndoEdit objects. We instantiate these so that the changes
     * found in the file on disk can be reproduced in memory by calling redo()
     * on them. REDO, not UNDO!
     */
    ...
    private void scanMetaData(MetaData inMem, MetaData inTemp, MetaData onDisk) {
        ...
        for (String key : inTemp) {
            ...
            if (vod == null) {
                ...
                List<String> vit = inTemp.getData(key);
                if (!vod.equals(vit)) {
    ...
    /**
     * Finds the entry in neu best fitting the specified entry in old. If no
     * entries get a score above zero, an entry is still returned.
     *
