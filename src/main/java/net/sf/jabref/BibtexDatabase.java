...
public class BibtexDatabase {
    ...
    private final VetoableChangeListener listener
        ...
        public void vetoableChange(PropertyChangeEvent pce)
            ...
            if (pce.getPropertyName() == null) {
                ...
                BibtexEntry oldEntry = _entries.remove(pce.getOldValue());
                ...
                if (_entries.get(pce.getNewValue()) != null) {
                    _entries.put((String) pce.getOldValue(), oldEntry);
                    throw new PropertyVetoException("New ID already in use, please choose another",
                            pce);
                }
    ...
    /**
     * Returns a Set containing the keys to all entries.Use
     * getKeySet().iterator() to iterate over all entries.
     *
     * @return
     */
    ...
    /**
     * Returns an EntrySorter with the sorted entries from this base, sorted by
    ...
    public synchronized BibtexEntry[] getEntriesByKey(String key) {
        // Use the new key map if available, otherwise fall back
        if (_keyToEntryMap != null) {
            BibtexEntry entry = _keyToEntryMap.get(key);
            if (entry != null) {
        ...
        // Fallback to original implementation (should rarely be called)
        ...
        return entries.toArray(new BibtexEntry[0]);
    }

    // Update insertEntry to maintain the map
    public synchronized boolean insertEntry(BibtexEntry entry) throws KeyCollisionException {
        ...
        // MAINTAIN THE KEY MAP
        String citeKey = entry.getCiteKey();
        if (citeKey != null && !citeKey.isEmpty()) {
            _keyToEntryMap.put(citeKey, entry);
        }
    ...
    public synchronized BibtexEntry removeEntry(String id) {
        ...
        // REMOVE FROM KEY MAP
        String citeKey = oldValue.getCiteKey();
        if (citeKey != null) {
            _keyToEntryMap.remove(citeKey);
        }

        removeKeyFromSet(citeKey);
    ...
    public synchronized boolean setCiteKeyForEntry(String id, String key) {
        ...
        // UPDATE KEY MAP
        if (oldKey != null) {
            _keyToEntryMap.remove(oldKey);
        }
        if (key != null && !key.isEmpty()) {
            _keyToEntryMap.put(key, entry);
        }
    ...
    /**
     * Returns a Set of keys to all BibtexString objects in the database.These
     * are in no sorted order.
     *
     * @return
     */
    ...
    /**
     * Returns a Collection of all BibtexString objects in the database.These
    ...
    /**
     * Resolves any references to strings contained in this field content, if
     * possible.
     *
     * @param content
     * @return
     ...
     * references.
     *
     * @param entries A collection of BibtexEntries in which all strings of the
     * form #xxx# will be resolved against the hash map of string references
     * stored in the databasee.
     *
     * @param inPlace If inPlace is true then the given BibtexEntries will be
     * modified, if false then copies of the BibtexEntries are made before
     * resolving the strings.
     *
     * @return a list of bibtexentries, with all strings resolved. It is
     * dependent on the value of inPlace whether copies are made or the given
     * BibtexEntries are modified.
     */
     ...
     * resolved against the hash map of string references stored in the
     * databasee.
     *
     * @param inPlace If inPlace is true then the given BibtexEntry will be
     * modified, if false then a copy is made using close made before resolving
     * the strings.
     *
     * @return a BibtexEntry with all string references resolved. It is
     * dependent on the value of inPlace whether a copy is made or the given
     * BibtexEntries is modified.
     */
    ...
    /**
     * If the label represents a string contained in this database, returns that
     * string's content. Resolves references to other strings, taking care not
     * to follow a circular reference pattern. If the string is undefined,
     * returns null.
     */
    ...
    private String resolveContent(String res, HashSet<String> usedIds) {
        ...
        if (res.matches(".*#[^#]+#.*")) {
            ...
            while ((next = res.indexOf("#", piv)) >= 0) {
                ...
                // We found the next string ref. Append the text up to it.
                if (next > 0) {
                ...
                if (stringEnd >= 0) {
                    ...
                    } else // The string was resolved, so we display its meaning only,
     ...
     * references in the field-value. Also, if a database is given, this
     * function will try to find values for unset fields in the entry linked by
     * the "crossref" field, if any.
     *
     ...
     * @return The resolved text or the original text if either the text or the
     * database are null
     */
