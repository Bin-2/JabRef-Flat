...
public class ImportFormatReader {
     ...
     *
     * <p>
     * Will return the first format according to the default-order of format
     * that matches the given ID.</p>
     *
     * @param cliId CLI-Id
    ...
    public List<BibtexEntry> importFromFile(ImportFormat importer, String filename, OutputPrinter status) throws IOException {
        ...
        InputStream recognitionStream = null;
        try {
            recognitionStream = new FileInputStream(file);
            if (!importer.isRecognizedFormat(recognitionStream)) {
        ...
        try {
            ...
            return importer.importEntries(importStream, status);
        } finally {
     ...
     *
     * <p>
     * Elements are in default order.</p>
     *
    ...
    /**
     * Expand initials, e.g. EH Wissler -> E. H. Wissler or Wissler, EH ->
     * Wissler, E. H.
     *
     * @param name
...
//------------------------------------------------------------------------------
    ...
    /**
     * Receives an ArrayList of BibtexEntry instances, iterates through them,
     * and removes all entries that have no fields set. This is useful for
     * rooting out an unsucessful import (wrong format) that returns a number of
     * empty entries.
     */
