 ...
 * formatted in JabRef style. The reformatting must undo all formatting done by
 * JabRef when writing the same fields.
 */
public class FieldContentParser {
     ...
     *
     * @param content StringBuffer containing the field to format. key contains
     * field name according to field was edited by Kuehn/Havalevich
    ...
    public StringBuffer format(StringBuffer content, String key) {
        ...
        while (i < content.length()) {
            ...
            } else if (c == '\t') // Remove all tab characters that aren't associated with a line break.
            {
                content.deleteCharAt(i);
            } else {
                i++;
    ...
    /**
     * Formats field contents for output. Must be "symmetric" with the parse
     * method above, so stored and reloaded fields are not mangled.
     *
     * @param in
     * @param wrapAmount
