...
public final class PreviewPanel extends JPanel implements VetoableChangeListener, SearchTextListener, EntryContainer {
     ...
     * @param entry (may be null) If given this entry is shown otherwise you
     * have to call setEntry to make something visible.
     * @param panel (may be null) If not given no toolbar is shown on the right
     * hand side.
     * @param metaData (must be given) Used for resolving pdf directories for
     * links.
     ...
     * links.
     * @param layoutFile (must be given) Used for layout
     * @param withPDFPreview if true, a PDF preview is included in the
     * PreviewPanel
     */
    public PreviewPanel(BibtexDatabase database, BibtexEntry entry,
     ...
     * @param layoutFile (must be given) Used for layout
     * @param withPDFPreview if true, a PDF preview is included in the
     * PreviewPanel. The user can override this setting by setting the config
     * setting JabRefPreferences.PDF_PREVIEW to false.
     */
...
//    }
    ...
    JToolBar createToolBar() {
        ...
        Action _printAction = getPrintAction();
        Action _closeAction = getCloseAction();
        ...
        JToolBar tlb = new JToolBar(JToolBar.VERTICAL) {
            ...
            public void updateUI() {
                ...
                // Reapply custom settings after UI update
        ...
        am.put("close", _closeAction);

        im.put(prefs.getKey("Print entry preview"), "print");
        am.put("print", _printAction);

        tlb.setFloatable(false);

        // Add actions (and thus buttons)
        tlb.add(_closeAction);
        tlb.addSeparator();
        tlb.add(_printAction);
