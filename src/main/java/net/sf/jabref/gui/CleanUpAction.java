...
public class CleanUpAction extends AbstractWorker {
    ...
    public final static String AKS_AUTO_NAMING_PDFS_AGAIN = "AskAutoNamingPDFsAgain",
    ...
    /**
     * Converts the text in 1st, 2nd, ... to real superscripts by wrapping in
     * \textsuperscript{st}, ...
     */
    ...
    /**
     * Removes the http://... for each DOI Moves DOIs from URL and NOTE filed to
     * DOI field
     *
     * @param ce
     */
    ...
    private void doRenamePDFs(BibtexEntry entry, NamedCompound ce) {
        ...
        for (int i = 0; i < flModel.getRowCount(); i++) {
            ...
            if (new File(newPath).exists()) // we do not overwrite files
