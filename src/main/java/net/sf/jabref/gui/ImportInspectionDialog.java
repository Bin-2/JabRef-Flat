...
public class ImportInspectionDialog extends JDialog implements ImportInspector, OutputPrinter {
    ...
    public void addEntries(Collection<BibtexEntry> entries) {
        ...
        for (BibtexEntry entry : entries) {
            ...
            // status:
            BibtexEntryType activeType
                    = BibtexEntryType.getType(entry.getType().getName());

            if (activeType != null && activeType != entry.getType()) {
                entry.setType(activeType);
            }
     ...
     * time.
     * <p/>
     * Synchronizes on this.entries to prevent conflict with threads that modify
     * the entry list.
     */
