...
/**
 * Created by IntelliJ IDEA. User: alver Date: Oct 31, 2005 Time: 10:46:03 PM To
...
public class DatabasePropertiesDialog extends JDialog {
    ...
    public void setValues() {
        ...
        List<String> storedSaveOrderConfig = metaData.getData(SAVE_ORDER_CONFIG);
        boolean selected;
        ...
        List<String> fileD = metaData.getData(Globals.prefs.get("userFileDir"));
        if (fileD == null) {
            fileDir.setText("");
        } else {
            // Better be a little careful about how many entries the List has:
            if (fileD.size() >= 1) {
                fileDir.setText((fileD.get(0)).trim());
            }
        }

        List<String> fileDI = metaData.getData(Globals.prefs.get("userFileDirIndividual")); // File dir setting
        List<String> fileDIL = metaData.getData(Globals.prefs.get("userFileDirInd_Legacy")); // Legacy file dir setting for backward comp.
        if (fileDI == null) {
        ...
        List<String> pdfD = metaData.getData("pdfDirectory");
        if (pdfD == null) {
            pdfDir.setText("");
        } else {
            // Better be a little careful about how many entries the List has:
        ...
        List<String> psD = metaData.getData("psDirectory");
        if (psD == null) {
            psDir.setText("");
        } else {
            // Better be a little careful about how many entries the List has:
    ...
    public void storeSettings() {
        ...
        if (saveAsConfiguredGlobally.isSelected()) {
            ...
            List<String> serialized = saveOrderConfig.getList();
        ...
        if (oldSaveOrderConfig == newSaveOrderConfig) {
            ...
            saveOrderConfigChanged = !oldSaveOrderConfig.getList().equals(newSaveOrderConfig.getList());
        }
