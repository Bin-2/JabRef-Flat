...
//    	}
    ...
    /**
     * This method is used if the focus listener should call another focus
     * listener after finishing. This is needed because the autocomplete
     * listener must run before the focus listener responsible for storing the
     * current edit.
     *
    ...
    /**
     * This setting determines whether the autocomplete listener should consume
     * the Enter key stroke when it leads to accepting a completion. If set to
     * false, the JTextComponent will receive the Enter key press after the
     * completion is done. The default value if true.
     *
     * @param t true to indicate that the Enter key should be consumed, false
    ...
    /**
     * If user cancels autocompletion by a) entering another letter than the
     * completed word (and there is no other auto completion) b) space the
     * casing of the letters has to be kept
     *
     * Global variable "lastBeginning" keeps track of typed letters. We rely on
     * this variable to reconstruct the text
     *
     * @param wordSeperatorTyped indicates whether the user has typed a white
     * space character or a
     */
    ...
    /**
     * Start a new completion attempt (instead of treating a continuation of an
     * existing word or an interrupt of the current word)
     */
    private void startCompletion(StringBuffer currentword, KeyEvent e) {
        ...
        if ((completed != null) && (completed.length > 0)) {
            ...
            if ((sno == null) || (cWord == null) || (cWord.length() == 0) || (sno.length() < cWord.length())) {
                resetAutoCompletion();
                return;
            }
    ...
    protected int findNamePositionStatus(JTextComponent comp) {
        ...
        try {
            ...
            int andIndex = upToCaret.lastIndexOf(" and ");
            if (andIndex >= 0) {
                upToCaret = upToCaret.substring(andIndex + " and ".length());
            }
