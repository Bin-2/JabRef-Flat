...
public class FindUnlinkedFilesDialog extends JDialog {
     ...
     * Used via reflection in {@link net.sf.jabref.imports.DatabaseFileLookup}
    ...
    /**
     * Restores the location and size of this dialog from the persistent
     * storage.
     */
     ...
     * preferences.
     *
     * @param lastSelectedDirectory directory that is used as the working
     * directory in this view.
     */
     ...
     * <br>
     * If the user cancels the "Open file" dialog, this method returns null.
     * <br>
     * <br>
     ...
     * <br>
     * When the import has finished, the
     * {@link #importFinishedHandler(int, java.util.List)} is invoked.
     */
     ...
     *
     * @param rootNode The root of the file structure as the result of the
     * search.
     */
     ...
     * @param container The {@link Container}, to which the component will be
     * added.
     * @param component An AWT {@link Component}, that will be added to the
     * container.
     * @param fill A constant describing the fill behaviour (see
     * {@link GridBagConstraints}). Can be <code>null</code>, if no filling
     * wants to be specified.
     * @param anchor A constant describing the anchor of the element in its
     * parent container (see {@link GridBagConstraints}). Can be
     ...
     * <br>
     * Initially, the root node is <b>not</b> visible, so that the tree appears
    ...
    private void createFileTypesCombobox() {
        ...
        FileFilter[] fileFilterArray = fileFilterList.toArray(new FileFilter[0]);
        comboBoxFileTypeSelection = new JComboBox<>(fileFilterArray);
    ...
    private void createEntryTypesCombobox() {
        ...
        list.addAll(entryTypes.values().stream()
                .map(BibtexEntryTypeWrapper::new)
                .collect(Collectors.toList()));

        comboBoxEntryTypeSelection = new JComboBox<>(list.toArray(new BibtexEntryTypeWrapper[0]));
    }
