...
public class SidePaneManager {
    ...
    /**
     * Safely update all side pane components
     */
    private void updateAllComponents() {
        // Update all registered components
        for (Map.Entry<String, SidePaneComponent> entry : components.entrySet()) {
            SidePaneComponent component = entry.getValue();
    ...
    /**
     * Safely update a single side pane component
     */
    private void updateComponentSafely(SidePaneComponent component, String componentName) {
        if (component != null) {
            try {
                // Call the component's own update method which handles JGoodies properly
                component.updateUIForThemeChange();

            } catch (Exception e) {
                System.err.println("Failed to update side pane component '" + componentName + "': " + e.getMessage());
