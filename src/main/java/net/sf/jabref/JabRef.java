...
public final class JabRef {
    ...
    public List<ParserResult> processArguments(String[] args, boolean initialStartup) {
     ...
     * any GUI.
     *
     * @param fetchCommand A string containing both the fetcher to use (id of
     * EntryFetcherExtension minus Fetcher) and the search query, separated by a
     * :
...
//            UIManager.put("TextField.font", UIManager.getFont("defaultFont"));
    ...
    public void openWindow(List<ParserResult> loaded) {
    ...
    /**
     * Will open a file (like importFile), but will also request JabRef to focus
     * on this database
     *
    ...
    // Global Mouse Listener for Click Events
    private void setupSimpleClickLogger() {
            ...
            public void eventDispatched(AWTEvent event) {
                if (event instanceof MouseEvent && event.getID() == MouseEvent.MOUSE_CLICKED) {
                    ...
                    if (source instanceof java.awt.Component) {
                        ...
                        System.out.printf("Clicked: %s", comp.getClass().getName());

                        if (comp.getName() != null) {
                            System.out.printf(" [%s]", comp.getName());
                        }

                        java.awt.Component parent = comp.getParent();
                        if (parent != null) {
                            System.out.printf(" -> Parent: %s", parent.getClass().getName());
                        }
